...
class Bola:
    ...
    def colisao(self, jogadorA, jogadorB, maxVel, amplitudeAngulo):
        if self.x <= jogadorA.x + 25 * valor_horizontal_redimensional and self.y >= jogadorA.y and self.y <= jogadorA.y + 100 * valor_vertical_redimensional:
            if jogadorA.tangivel == True:
                pygame.mixer.music.load(random.choice(['./audios/colisao_jogador1.mp3', './audios/colisao_jogador2.mp3','./audios/colisao_jogador3.mp3', './audios/colisao_jogador4.mp3','./audios/colisao_jogador5.mp3','./audios/colisao_jogador6.mp3']))  # Escolhe um audio aleatório para tocar
        ...
        if self.x >= jogadorB.x - 10 * valor_horizontal_redimensional and self.y >= jogadorB.y and self.y <= jogadorB.y + 100 * valor_vertical_redimensional:
            if jogadorB.tangivel == True:
                pygame.mixer.music.load(random.choice(
                    ['./audios/colisao_jogador1.mp3', './audios/colisao_jogador2.mp3', './audios/colisao_jogador3.mp3', './audios/colisao_jogador4.mp3', './audios/colisao_jogador5.mp3', './audios/colisao_jogador6.mp3']))  # Escolhe um audio aleatório para tocar pygame.mixer.music.play()
    ...
    def resetar(self):
        ...
        self.movY = 0 * valor_vertical_redimensional
        pygame.mixer.music.load('./audios/ponto.mp3')
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_ESCAPE:
                    ...
                    pygame.draw.rect(tela, cores['ciano'], (
                    350 * valor_horizontal_redimensional, 375 * valor_vertical_redimensional,
                    100 * valor_horizontal_redimensional, 50 * valor_vertical_redimensional))
                    ...
                    while pausado:
                        for evento in pygame.event.get():
                            ...
                            if evento.type == pygame.MOUSEBUTTONDOWN:
                                ...
                                if evento.button == 1 and 350 * valor_horizontal_redimensional < evento.pos[
                                    0] < 450 * valor_horizontal_redimensional and 375 * valor_vertical_redimensional < \
                                        evento.pos[1] < 425 * valor_vertical_redimensional:
                                    pausado = False
...
def tela_dificuldade_bot():
    ...
    while rodando:
        ...
        if desenhar_botao("Fácil", 300, 250, 200, 50, cores['branco'], cores[
            'ciano']):  # Se o botão for clicado, a função jogo será chamada com os parâmetros: 1 jogador, velocidadeBot = 0.5, maxVel = 2 e angulo = 50
            jogo(1, 0.6, 5, 50, 1.25, 1)
            rodando = False

        if desenhar_botao("Médio", 300, 350, 200, 50, cores['branco'], cores[
            'ciano']):  # Se o botão for clicado, a função jogo será chamada com os parâmetros: 1 jogador, velocidadeBot = 0.75, maxVel = 4 e angulo = 50
            jogo(1, 0.75, 5, 70, 1.75, 1)
            rodando = False

        if desenhar_botao("Impossível", 300, 450, 200, 50, cores['branco'], cores[
            'ciano']):  # Se o botão for clicado, a função jogo será chamada com os parâmetros: 1 jogador, velocidadeBot = 0.85, maxVel = 5 e angulo = 100
